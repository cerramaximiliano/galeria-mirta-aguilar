...
import { Award, Calendar, MapPin, Loader2, Image, FileText } from 'lucide-react';
...
const Biography = () => {
  ...
  const openModal = (url, title, caption = '') => {
    setModalUrl(url);
    setModalTitle(title);
  ...
  return (
    <>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {highlight.externalUrl && (
                      <button
                        onClick={() => openModal(highlight.externalUrl, `${highlight.year} - ${highlight.achievement}`, highlight.externalUrlCaption)}
                  ...
                  {(exhibition.externalUrl || exhibition.catalogUrl) && (
                    <div className="flex flex-wrap gap-4 mt-3 pt-3 border-t border-gallery-100">
                      {exhibition.externalUrl && (
                        <button
                          onClick={() => openModal(exhibition.externalUrl, `${exhibition.title} - ${exhibition.year}`, exhibition.externalUrlCaption)}
                      ...
                      {exhibition.catalogUrl && (
                        <button
                          onClick={() => openModal(exhibition.catalogUrl, `Catálogo - ${exhibition.title}`, exhibition.catalogUrlCaption)}
                          className="inline-flex items-center gap-1 text-sm text-accent hover:text-accent/80 transition-colors"
                        >
                ...
                >
                  ...
                  {(award.externalUrl || award.certificateUrl) && (
                    <div className="flex flex-wrap justify-center gap-3 mt-3 pt-3 border-t border-accent/20">
                      {award.externalUrl && (
                        <button
                          onClick={() => openModal(award.externalUrl, `${award.title} - ${award.year}`, award.externalUrlCaption)}
                          className="inline-flex items-center gap-1 text-sm text-accent hover:text-accent/80 transition-colors"
                        >
                          <Image className="h-3.5 w-3.5" />
                          Ver más
                      ...
                      {award.certificateUrl && (
                        <button
                          onClick={() => openModal(award.certificateUrl, `Certificado - ${award.title}`, award.certificateUrlCaption)}
                          className="inline-flex items-center gap-1 text-sm text-accent hover:text-accent/80 transition-colors"
                        >
                          <FileText className="h-3.5 w-3.5" />
                          Certificado
      ...
      {/* Modal para mostrar imágenes/archivos */}
      <MediaModal
        isOpen={modalOpen}
        onClose={closeModal}
        url={modalUrl}
